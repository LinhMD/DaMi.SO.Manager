...
public class OrderDetailController(IUnitOfWork work) : ControllerBase
{
    ...
    {
        var orderMaster = await work
                                .Get<OrderMaster>()
                                .IncludeAll()
                                .Include(f => f.OrderDetails)
                                .Include(o => o.OrderForm)
                                .Where(f => f.RowUniqueId == OrderId!)
                                .FirstOrDefaultAsync();

        var taxCode = await work.Get<ViwCustomer>()
                            .Find(c => c.CustomerId == orderMaster!.CustomerId)
                            .Select(c => c.TaxCode)
                            .FirstOrDefaultAsync();
        ...
        var item = await work.Get<ViwFullItem>()
                        .Find(f => f.ItemId == ItemIdSelect)
                        .FirstOrDefaultAsync();
        if (item is not null)
        {
            ...
            orderDetailSimpleView.NumOfData = item.DefNumOfData;
            orderDetailSimpleView.NumOfPc = item.DefNumOfPc;
            orderDetailSimpleView.NumOfInv = item.DefNumOfInv;
            orderDetailSimpleView.NumOfMonth = item.DefNumOfMonth;
            orderDetailSimpleView.NumOfUser = item.DefNumOfUser;
            orderDetailSimpleView.NumOfTaxCode = item.DefNumOfTaxCode;
    ...
    public async Task<IResult> PostNew([FromForm] OrderDetailSimpleView orderDetailNew)
    {
        try
        {
            var item = await work.Get<ViwItem>()
                            .Find(x => x.ItemId == orderDetailNew.ItemId)
                            .FirstOrDefaultAsync();
            if (item is not null)
            {
            ...
            var orderDetail = orderDetailNew.Adapt<OrderDetail>();
        ...
        catch (ModelValueInvalidException e)
        {
            var orderMaster = await work.Get<OrderMaster>()
                                    ...
                                    .Include(f => f.OrderDetails)
                                    .Include(o => o.OrderForm)
                                    .Where(f => f.RowUniqueId == orderDetailNew.OrderId!)
                                    .FirstOrDefaultAsync();
    ...
    public async Task<IResult> PostEdit([FromForm] OrderDetailSimpleView orderDetail, Guid guid)
    {
        ...
        (
            ...
            &&
            (
                orderDetail.OriginalDiscAmount == 0
                || orderDetail.OriginalDiscAmount is null
            )
        )
        {
        ...
        try
        {
            ...
            (
                work,
                await work.Get<OrderDetail>().GetAsync<OrderDetailSimpleView>(guid),
                orderMaster,
                null,
                ViewMode.Detail
            );
        ...
        catch (ModelValueInvalidException e)
        {
            ...
            var orderDetailSimpleView = await work.Get<OrderDetail>().GetAsync<OrderDetailSimpleView>(guid);
    ...
    {
        var fullItems = await work.Get<ViwFullItem>()
                                    .Find(f => orderMaster != null && f.OrderFormId == orderMaster.OrderFormId)
                                    .ToDictionaryAsync(f => f.ItemId);
        return new RazorComponentResult(typeof(OrderDetailRow), new
        {
            Model = new OrderDetailModifyModel
            {
                OrderDetails = [],
                ItemTypes = await work.Get<ViwItemType>()
                                    .GetAll()
                                    .ToListAsync(),
                TaxCodes = await work.Get<TaxCode>()
                                .Find(t => t.CustomerId == orderMaster!.CustomerId)
                                .ToListAsync(),
            ...
            detail = orderDetail,
            Errors = e,
            FullItem = fullItems.ContainsKey(orderDetail?.ItemId ?? "")
                        ? fullItems[orderDetail?.ItemId ?? ""]
                        : new ViwFullItem(),
            Form = orderMaster?.OrderForm,
            Order = orderMaster.Adapt<OrderMasterDetailView>(),
            Action = true,
            DBClickAction = viewMode == ViewMode.Detail,
            Customer = await work.Get<ViwCustomer>()
                            .Find(t => t.CustomerId == orderMaster!.CustomerId)
                            .FirstOrDefaultAsync()
        });
    }
